...
class _HtmlDataWidgetState extends State<HtmlDataWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    return HtmlWidget(
        convertCustomAudioTags(widget.text),
        ...
        if (element.localName == 'img') {
          ...
          if (srcAttribute != null) {
            return FutureBuilder<File?>(
              future: initDirectory(srcAttribute),
                ...
                if (snapshot.connectionState == ConnectionState.waiting) {
                  ...
                  if (file != null) {
                    return Image.file(
                      ...
                      fit: BoxFit.contain,
                      height: int
                          .parse(width.toString())*2
                          .w,
                      width: int
                          .parse(height.toString())*2
                          .h,
                    ); // Display the image from the file
                  } else {
                    return Text('No image'); // If the file doesn't exist
                  }
                }
              },
            );
              ...
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  ...
                  if (file != null) {
                    ...
                    return const Text('No Audio'); // If the file doesn't exist
                  }
                }
              },
            );
  ...
  Future<File?> initDirectory(String srcAttribute) async {
    if(appSupportDir==null) {
      return null;
    }
    //todo remove
    //srcAttribute = '420-ג.jpg';
    String path = '${appSupportDir!.path}/${Constants.quizPath}/${widget.quizId}/$srcAttribute';
    File file = File(path);
    //todo remove
